...
def update_adj(lookup, brackets, adj, is_reversed, front, back, direction, end, d):  # Time: O(K)
...
def find_shortest_path(pair, lookup, brackets, t):  # Time: O(KlogK)
    result = []
...
def build(PRG, pair, lookup, tree, node):  # Time: O(KlogK)
    ...
    for i in partition_idxs:
        if i in (-1, len(brackets)):  # virtual brackets we added
            continue
        lookup[brackets[i]] = find_shortest_path(pair, lookup, brackets, brackets[i])  # Time: O(KlogK)
...
def query(PRG, pair, lookup, tree, node, s, e):  # Time: O(K * (logK)^2) for lazy build, O(QlogK) for query, run at most O(KlogK) in each depth, at most O(logK) depth
...
def emacspp():
    ...
    pair = find_pair(PRG)
    lookup, tree = init_dist(L, R, P, pair), [[range(len(PRG)), -1, len(PRG)]]
    return sum(query(PRG, pair, lookup, tree, 0, s-1, e-1) for s, e in izip(S, E))
